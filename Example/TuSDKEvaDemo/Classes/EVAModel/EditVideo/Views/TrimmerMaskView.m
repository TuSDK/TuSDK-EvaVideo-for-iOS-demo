...
- (void)setStartProgress:(double)startProgress {
    ...
    _leftThumb.translation = CGPointMake((startProgress - _startProgress) * contentWidth, 0);
    self.maskRect = [self maskRectWithThumb:_leftThumb];
...
- (void)setEndProgress:(double)endProgress {
    _panBeganMaskRect = self.maskRect;
    const CGFloat contentWidth = CGRectGetWidth(_maskLayer.bounds);
    _rightThumb.translation = CGPointMake((endProgress - _endProgress) * contentWidth, 0);
    self.maskRect = [self maskRectWithThumb:_rightThumb];
